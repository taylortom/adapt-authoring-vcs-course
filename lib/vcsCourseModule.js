...
class VCSCourseModule extends AbstractModule {
  ...
  */
  async onAction(action, p1, p2) {
    ...
    if(revision.changes.length) { // note we access the MongoDB API directly to avoid an infinite event loop
      return this.db.getCollection('revisions').updateOne({ itemId: courseId }, {
        $set: { itemId: courseId, itemCollection: this.content.collectionName, [`revisions.${now.getTime()}`]: revision }
      }, { upsert: true });
    }
  }
  /**
  ...
  */
  ...
  async getRevisions(courseId) {
    const query = { itemId: this.db.ObjectId.parse(courseId) };
    const mongoOpts = { limit: 100, sort: { timestamp: -1 } };
    return this.db.find(this.collectionName, query, {}, mongoOpts);
  }
  /**
  ...
  */
  async getRevisionsHandler(req, res, next) {
    ...
    try {
      res.json({
        course: await this.content.find({ _id }),
        revisions: await this.getRevisions(_id)
      });
    } catch(e) {
