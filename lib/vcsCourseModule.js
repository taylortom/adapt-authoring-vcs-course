...
class VCSCourseModule extends AbstractModule {
  ...
  */
  ...
  /**
  * Reverts a number of changes
  * @param {Array} changes The changes to revert
  */
  async undoChangesRecursive(changes) {
    const { diff, target } = changes.shift();
    const [currentData] = await this.content.find({ _id: target._id });
    if(currentData) {
      const revertedData = jsondiffpatch.unpatch(currentData, diff);

      if(revertedData === undefined) {
        await this.content.delete({ _id: target._id });
      } else {
        const parsed = JSON.parse(JSON.stringify(revertedData)); // to convert any ObjectId instances to string
        await this.content.replace({ _id: target._id }, parsed, { schemaName: target._type }, { upsert: true });
      }
  }
