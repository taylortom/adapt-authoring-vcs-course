  ...
  */
  async onAction(action, p1, p2) {
    ...
    data.forEach(([oldData, newData]) => {
      ...
      const now = new Date();
      revisions[`revisions.${now.getTime()}`] = {
        action,
    ...
    if(Object.keys(revisions).length) { // note we access the MongoDB API directly to avoid an infinite event loop
      return this.db.getCollection('revisions').updateOne({ itemId: courseId }, {
        $set: { itemId: courseId, itemCollection: this.content.collectionName, ...revisions }
      }, { upsert: true });
    }
  }
  /**
