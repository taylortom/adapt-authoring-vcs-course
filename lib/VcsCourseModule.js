 ...
 */
export default class VcsCourseModule extends AbstractModule {
  /** @override */
  async init() {
    const [content, jsonschema, mongodb, ui] = await this.app.waitForModule('content', 'jsonschema', 'mongodb', 'ui');
    /**
    ...
    /**
     * Reference to the MongoDBModule instance for convenience
     * @type {MongoDBModule}
     */
    this.db = mongodb;
    /**
     * Reference to the JsonSchemaModule instance for convenience
  ...
  async initRouter() {
    ...
    auth.secureRoute(`${this.router.path}/revisions/:_id`, 'get', ['read:revisions']);
    auth.secureRoute(`${this.router.path}/revert/:_id`, 'post', ['write:revisions']);
  }
  /**
